...
- FlexPendant 式程序编辑器：「添加指令」只列 MoveJ/MoveL 并插入 `*,v1000,z50,tool0` 未示教占位（程序不可运行）；双击指令行或「编辑 → 更改选定内容」打开参数面板改写目标点/速度/转弯区，「修改位置」为工具栏常驻按钮；注释/取消注释、Change to MoveJ↔MoveL、撤销/重做（3 步）；剪切/复制/粘贴直接用编辑器系统剪贴板（Ctrl+X/C/V），不设菜单项。Program Data 暂不维护第二份点位存储。

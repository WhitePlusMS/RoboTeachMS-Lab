...
- 非嵌套 `IF`/`ELSEIF`/`ELSE`/`ENDIF` 分支；条件、赋值与运动共用同一执行计划
...
当前刻意只覆盖基础逻辑教学闭环：模块级 `VAR num/bool`、标量赋值与表达式、非嵌套条件分支，以及分支内既有 `MoveJ`/`MoveL`。运行值由现有 ProgramExecutor 快照持有，源码、Program Data、PP/MP 和运动规划不建立平行状态。

当前未实现：嵌套控制流、循环、`TEST/CASE`、PROC/FUNC 调用、I/O、复合数据赋值、变量驱动 `Offs`/`RelTool`、完整调试器能力和完整 RAPID 编译器语义。超出子集的语法会给出诊断并阻止部分执行。

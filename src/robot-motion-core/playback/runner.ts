import {
  ...
  type TimedJointWaypoint,
} from './joint-trajectory-sampler.ts'
import { DEFAULT_MOTION_CONFIG, lerpJoints } from './smoothing.ts'
import type { JointAngles } from '../../robot-geometry/robot-types.ts'
import type { MotionConfig } from './playback-config.ts'
...
export interface MotionRunner {
  ...
  startTrajectory: (waypoints: readonly TimedJointWaypoint[]) => Promise<MotionResult>
  /** 连续点动替换为从实际当前状态规划的完整路径，复用单一帧循环。 */
  appendTrajectory: (waypoints: readonly TimedJointWaypoint[]) => Promise<MotionResult>
  pause: () => void
  resume: () => void
  stop: () => void
...
export function createMotionRunner(options: MotionRunnerOptions): MotionRunner {
  ...
  let trajectory: readonly TimedJointWaypoint[] = []
  ...
  function runTrajectory(): void {
    const elapsed = effectiveElapsed()
    options.setJoints(sampleJointTrajectory(trajectory, elapsed))
    if (elapsed < trajectory[trajectory.length - 1].timeMs) requestFrame(runTrajectory)
    else finishMotion()
  }
  ...
  function startTrajectory(waypoints: readonly TimedJointWaypoint[]): Promise<MotionResult> {
    validateJointTrajectory(waypoints)
    // 计划从提交时的实际关节开始；不把过去的目标锚点拼接到轨迹头部。
    trajectory = waypoints.map((point) => ({
      timeMs: point.timeMs,
      jointsDeg: [...point.jointsDeg] as JointAngles,
    }))
    startTime = options.clock.now()
    totalPausedTime = 0
    pausedAt = status === 'paused' ? startTime : null
  ...
  return {
    ...
    startSpeedLimited,
    startTrajectory,
    appendTrajectory: startTrajectory,
    pause,
    resume,
    stop,
    getStatus,

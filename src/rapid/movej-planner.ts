...
export function planMoveJ(
  ...
  const joints = solveIK(
    robTargetToPose(movej.target),
    currentJoints,
    model,
    {},
    jointRanges,
  ...
  if (!joints) {
    return { ok: false, error: { kind: 'unreachable', message: '目标姿态不可达或逆解未收敛' } }
  }

  // 若唯一解被夹在关节范围边界，说明到达物理极限，判别为 joint-limit。
  if (isJointAtLimit(joints, jointRanges)) {
    return {
      ok: false,
      error: {
        kind: 'joint-limit',

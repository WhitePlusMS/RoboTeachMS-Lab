...
describe('Ticket 03 — 表达式错误精确诊断并阻止执行', () => {
  const decl = '    CONST robtarget pBase := [[100,200,300],[1,0,0,0],[0,0,0,0],[9E9,9E9,9E9,9E9,9E9,9E9]];\n'

  it('未定义基准目标报 undefined-symbol', () => {
    const result = parseRapidProgram(`MODULE T\n${decl}    PROC main()\n        MoveJ Offs(nope,1,2,3),v100,fine,tool0;\n    ENDPROC\nENDMODULE\n`)
    expect(result.diagnostics.some((d) => d.code === 'undefined-symbol' && d.message.includes('nope'))).toBe(true)
    expect(result.canExecute).toBe(false)
  })

  it('参数数量错误（Offs 2 个）报 invalid-data', () => {
    const result = parseRapidProgram(`MODULE T\n${decl}    PROC main()\n        MoveJ Offs(pBase,1,2),v100,fine,tool0;\n    ENDPROC\nENDMODULE\n`)
    expect(result.diagnostics.some((d) => d.code === 'invalid-data' && d.message.includes('Offs 需要 3'))).toBe(true)
    expect(result.canExecute).toBe(false)
  })

  it('RelTool 参数数量不是 3/6 报 invalid-data', () => {
    const result = parseRapidProgram(`MODULE T\n${decl}    PROC main()\n        MoveJ RelTool(pBase,1,2,3,4),v100,fine,tool0;\n    ENDPROC\nENDMODULE\n`)
    expect(result.diagnostics.some((d) => d.code === 'invalid-data' && d.message.includes('RelTool 需要 3 或 6'))).toBe(true)
    expect(result.canExecute).toBe(false)
  })

  it('非数值参数报 invalid-data', () => {
    const result = parseRapidProgram(`MODULE T\n${decl}    PROC main()\n        MoveJ Offs(pBase,x,0,0),v100,fine,tool0;\n    ENDPROC\nENDMODULE\n`)
    expect(result.diagnostics.some((d) => d.code === 'invalid-data' && d.message.includes('数字'))).toBe(true)
    expect(result.canExecute).toBe(false)
  })
...
describe('Ticket 03 — Offs 沿对象轴而非世界轴（旋转 wobj 下）', () => {
  it('旋转 wobj 的 uframe 下，Offs(10,0,0) 的世界位移 = 对象局部 +X 方向（非世界 +X）', () => {
    ...
    const base: RobTarget = { trans: [100, 200, 300], rot: [1, 0, 0, 0], robconf: [0, 0, 0, 0], extax: [9e9, 9e9, 9e9, 9e9, 9e9, 9e9] }
    ...
    const delta = [worldOffs[0] - worldBase[0], worldOffs[1] - worldBase[1], worldOffs[2] - worldBase[2]]
    // Offs 沿对象轴：世界位移 = Ry(30°)·[10,0,0] = [8.660, 0, -5]，而非世界 +X 的 [10,0,0]。
    expect(delta[0]).toBeCloseTo(8.660, 2)

...
import { buildRapidMotionRequest } from './motion-requests.ts'
...
ENDMODULE`)
    ...
    for (const instruction of instructions) {
      const result = buildRapidMotionRequest(
        instruction,
        'off',
      ...
      if (result.ok) {
        expect(result.request.intent.kind).toBe(instruction.kind === 'movej' ? 'cartesian-target' : 'linear-path')

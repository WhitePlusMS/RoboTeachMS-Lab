...
import { planStrictCandidateGraph } from './cartesian-candidate-graph.ts'
...
describe('Cartesian candidate graph', () => {
  it('非奇异路径使用整层候选规划并保持无 180° 腕跳', () => {
    ...
    for (const joints of path.waypoints) {
      maxWristStep = Math.max(maxWristStep, Math.abs(joints[3] - previous[3]), Math.abs(joints[5] - previous[5]))
      previous = joints
    }

...
describe('关节控制状态', () => {
  it('手动输入和步进都会限制在关节范围内', () => {
    expect(clampJointAngle(1, 999, KUKA_JOINT_RANGES)).toBe(45)
    expect(clampJointAngle(1, -999, KUKA_JOINT_RANGES)).toBe(-90)
    expect(setJointAngle(DEFAULT_JOINTS, 1, 999, KUKA_JOINT_RANGES)[1]).toBe(45)
    expect(adjustJointAngle([45, 0, 0, 0, 0, 0], 1, 1, 10, KUKA_JOINT_RANGES)[1]).toBe(10)
  })

  it('加减步进使用指定的角度幅度', () => {
    const start: JointAngles = [0, 0, 0, 0, 0, 0]
    expect(adjustJointAngle(start, 0, 1, 5, KUKA_JOINT_RANGES)[0]).toBe(5)
    expect(adjustJointAngle(start, 0, -1, 0.1, KUKA_JOINT_RANGES)[0]).toBe(-0.1)
  })

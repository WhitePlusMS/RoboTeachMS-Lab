import { ABB_BASE_TO_SCENE } from './abb-scene-transform.ts'
import { mat3Transpose } from '@/robot-geometry/math/rotation3d.ts'
import type { Pose } from '@/robot-geometry/robot-types.ts'
import { rotationMatrixToEulerZYX } from '@/robot-geometry/math/rotation3d.ts'
import {
 ...
 *
 * 两个方向的旋转统一从 ABB_BASE_TO_SCENE 派生，避免显示与拖拽逆换算各自维护矩阵。
 */

/** ABB → 场景的 3x3 旋转矩阵（同一 M，见 ABB_BASE_TO_SCENE 左上 3x3）。 */
const ABB_TO_SCENE_ROTATION = ABB_BASE_TO_SCENE.getRotation()

/** 场景 → ABB 的 3x3 旋转矩阵（M 的转置）。 */
const SCENE_TO_ABB_ROTATION = mat3Transpose(ABB_TO_SCENE_ROTATION)

...
import { createWorkerMotionPlannerClient } from './motion-planner-client.ts'
...
class FakeWorker {
  ...
  onmessage:
    ((event: MessageEvent<{ requestId: number; result: MotionPlanningResult }>) => void) | null =
    null
  ...
  postMessage(message: { requestId: number; request: MotionPlanningRequest }): void {
  ...
  complete(result: MotionPlanningResult): void {
...
describe('Core Worker adapter', () => {
  it('同步预判成功后只消费一次，状态变化或取消后的不同请求不能复用', async () => {
    Object.defineProperty(globalThis, 'Worker', { configurable: true, value: FakeWorker })
    const adapter = createWorkerMotionPlannerClient()
    const valid: MotionPlanningRequest = {
      ...request,
      intent: { kind: 'joint-target', targetJointsDeg: [1, 0, 0, 0, 30, 0] },
    }
    const preview = adapter.preview(valid)
    expect(preview.ok).toBe(true)
    adapter.cancel()
    expect(await adapter.plan(valid, 'gizmo')).toBe(preview)
    expect(await adapter.plan(valid, 'gizmo')).not.toBe(preview)
    adapter.preview(valid)
    const changed: MotionPlanningRequest = { ...valid, state: { jointsDeg: [2, 0, 0, 0, 30, 0] } }
    const result = await adapter.plan(changed, 'gizmo')
    expect(result?.ok && result.waypoints[0].jointsDeg[0]).toBe(2)
    adapter.dispose()
  })
  ...
  it('取消活动规划会终止旧 Worker，下一请求只进入新 transport', async () => {
    ...
    const secondRequest = {
      ...request,
      state: { jointsDeg: [1, 1, 1, 1, 30, 1] as MotionPlanningRequest['state']['jointsDeg'] },
    }

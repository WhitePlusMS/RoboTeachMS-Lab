...
> 调研性质：本文件仅作只读现状梳理，供后续与 ABB 官方 RAPID 能力对比。所有"支持 / 部分支持 / 不支持"结论均尽量标注 `src/rapid/rapid-parser.ts` 的类型、常量或函数依据；其中"规划/交接文档提到但未实现"的事项单独列出并标注来源，避免脆弱的行号引用与已实现能力混淆。
>
> 单一事实源结论：RAPID 源程序是点位与程序的唯一事实源；Program Data 视图由 `parseRapidProgram` 同一次解析派生，无第二份点位存储或平行 parser（见 `RapidParseResult.data`、`program-control.ts` 的相关注释）。
...
| 能力 | 状态 | 关键依据 |
| --- | --- | --- |
| 运动指令 MoveJ / MoveL | **支持** | parser `parseMainBody`/`parseMotion` |
| 数据声明 CONST / PERS robtarget | **支持**（详见 §3） | `parseRobTargetDeclaration` |
| 可选参数 `\\WObj:=wobj0` | **部分支持**（仅 `wobj` 且值仅 `wobj0`） | `parseMotion` 的可选参数分支 |
| 注释 `! ...` | **支持**（单行注释，词法层跳过） | `lex` |
| 模块 / 单一过程结构 `MODULE ... PROC main() ... ENDMODULE` | **部分支持**（仅单个 `main`，见 §3） | `parseRapidProgram`/`parseProcedure`/`parseMainBody` |
| 诊断 | **支持**（9 种代码，见 §4） | `RapidDiagnosticCode` |
| 源码定位粒度 | **支持**（sourceRange + 各 operands 的 operandRanges） | `RapidExecutableInstruction` |
...
| MoveC / MoveAbsJ | **不支持**（触发 unsupported-syntax，不解析） | `parseMainBody`/`skipUnsupportedControl` |
| 控制流 IF / WHILE / FOR | **不支持**（执行层不解释） | `parseMainBody`/`skipUnsupportedControl`；`map.md` |
| 非 main 过程 / 多过程 / 参数 | **不支持** | `parseProcedure`/`skipProcedureBody` |
| WaitTime / I/O / 通信 | **不支持** | 全仓库无实现（见 §5，grep 无命中） |
| Zone 过渡（非 fine） | **不支持**（仅 `fine`） | `parseRapidProgram` 的运动校验 |
| 自定义 tool / wobj / speeddata | **不支持**（仅 `tool0`/`wobj0`/`v50/v100/v200`） | `SPEEDS` 与 `parseRapidProgram` 的运动校验 |
...
- 词法 Token 仅四类：`identifier`、`number`、`symbol`、`eof`；符号仅 `[](),;\\` 与 `:=`（`TokenKind`、`lex`）。
- 注释：单行 `! ...（到行尾）`，在 `lex` 中直接跳过。**不支持块注释 / `(*(*)`**。
- 关键字与标识符均**大小写不敏感**匹配（`normalizeName`/`isKeyword`），但保留源码原始拼写。
- 模块结构：必须 `MODULE <name>` 开头、单个 `ENDMODULE` 结尾（`parseRapidProgram`）。**一个源程序只允许一个 MODULE**。`ENDMODULE` 后不允许任何尾随内容。

### 2.2 运动指令（核心）

- **MoveJ 与 MoveL**：仅这两种运动指令被 `parseMainBody` 识别为可执行。
- 指令形式固定为：`<MoveJ|MoveL> <targetName>,<speedName>,<zoneName>,<toolName>[\\WObj:=<wobjName>];`（`parseMotion`）。
- 每个操作数在结构上只接受**标识符引用**（名称），不直接接受内联字面量表达式（`expectIdentifier`）。
- 速度名称：只接受内置 `v50` / `v100` / `v200`（`SPEEDS`），其余报 `undefined-symbol`。
- zone 名称：只接受 `fine`（`parseRapidProgram` 的运动校验）。
- tool 名称：只接受 `tool0`（`parseRapidProgram` 的运动校验）。
- 可选参数：只接受 `\\WObj:=wobj0`（`parseMotion`）；其它可选参数报 `unsupported-option`。注意 `wobj0` 是语法层面唯一接受的名字，`tool0`/`wobj0`/`zone` 也只接受默认值。
- 位置参数是通过名称解析到**模块级 robtarget 符号表**（`resolveTargetReferences` 与 `SymbolEntry`）。

### 2.3 数据声明（Program Data）

- 存储关键字：`CONST`、`PERS`、以及 `TASK PERS`（`parseRobTargetDeclaration`）。**`VAR` 不支持**（报 `unsupported-syntax`）。
- 类型：只支持 `robtarget`；`KNOWN_UNSUPPORTED_TYPES` 中的其它类型报 `unsupported-option`。
- robtarget 字面量形状固定为 `[[trans3],[rot4],[robconf4],[extax6]]`，全部为数字；拒绝尾逗号、长度不符、非有限数（`parseRobTarget`/`parseTuple`）。
  - `trans`：3 项，毫米。
...
- 名称大小写不敏感但保留原始拼写；重复定义（大小写不敏感）报 `duplicate-symbol`（`parseRobTargetDeclaration`）。
- 每个声明的源码范围、名称范围、值字面量范围、以及每条引用范围都会被精确记录（`RapidProgramDataTarget`、`SymbolEntry.references`）。

### 2.4 过程/程序结构

- 只接受**一个无参数 `PROC main()`**（`parseProcedure`）。其他过程名、带参数 main、多 main 均报错（`unsupported-syntax`/`unsupported-option`/`duplicate-symbol`）。
- 非 main 过程的函数体被跳过（`skipProcedureBody`），因此过程中的 MoveJ/MoveL 不会被解析成可执行指令。
- 无 `main` 报 `missing-entrypoint`（`parseRapidProgram` 的入口检查）。
...
| 项 | 允许值 | 拒绝示例（源码依据） |
| --- | --- | --- |
| 运动指令 | `MoveJ`、`MoveL` | `MoveC`、`MoveAbsJ` → `unsupported-syntax`（`parseMainBody`） |
| 速度 | `v50`、`v100`、`v200` | `v999` → `undefined-symbol`（`SPEEDS` 与运动校验） |
| zone | `fine` | `z20` → `unsupported-option`（运动校验） |
| tool | `tool0` | `tool1` → `unsupported-option`（运动校验） |
| wobj 可选参数 | `\\WObj:=wobj0` | `\\WObj:=wobj1`、`\\speed:=...` → `unsupported-option`（`parseMotion`） |
...
| 数据存储 | `CONST`、`PERS`、`TASK PERS` | `VAR` → unsupported（`parseRapidProgram` 模块内容分支） |
| 数据类型 | `robtarget` | `KNOWN_UNSUPPORTED_TYPES` 中的其它类型 → `unsupported-option` |

---

## 4. 诊断能力

### 4.1 诊断代码（`RapidDiagnosticCode`）

| 代码 | 触发场景示例（file:line） |
| --- | --- |
| `lexical-error` | 无法识别的字符（`lex`） |
| `syntax-error` | 缺关键字/符号/ENDMODULE 等（`expectKeyword`/`expectSymbol`/模块闭合检查） |
| `unsupported-syntax` | 不支持的控制流/其它过程/MoveAbsJ/MoveC/多余 MODULE/未知语句（`skipUnsupportedControl`/`parseMainBody`） |
| `unsupported-option` | 非默认 zone/tool/wobj/可选参数/非 robtarget 类型（`parseMotion`/运动校验/`parseRobTargetDeclaration`） |
| `duplicate-symbol` | 重复 robtarget / 多个 main（`parseRobTargetDeclaration`/`parseProcedure`） |
| `undefined-symbol` | 未定义 robtarget / 不支持的速度（`resolveTargetReferences`/运动校验） |
| `invalid-data` | tuple 长度/尾逗号/非有限数（`parseTuple`） |
| `missing-module` | 源程序没有 `MODULE`（`parseRapidProgram` 入口检查） |
| `missing-entrypoint` | 无无参 main（`parseRapidProgram` 入口检查） |

**注意**：全部诊断 `severity` 恒为 `'error'`（无 warning 级别，`RapidDiagnostic`）。存在任一 error 时 `canExecute=false` 且 `program=[]`（即使部分指令语义正确也不执行，`parseRapidProgram` 的结果收口）。

### 4.2 定位粒度

- 每个诊断携带一个 `range: RapidSourceRange`，最小粒度到**行/列**（1-based，`RapidSourcePosition`）。
- 每条可执行指令除 `sourceRange` 外，还有每个运动操作数的精确范围 `operandRanges.{target,speed,zone,tool,wobj}`（`RapidExecutableInstruction`），供编辑器标记与教学回溯。
...
- **派生**：`RapidParseResult.data` 在**每次解析**时由同一份符号表派生：名称、存储（const/pers）、目标值、声明/名称/值范围、每条引用范围。存在 error 时仍暴露已识别数据供只读浏览。
...
| 能力 | 状态 | 依据 |
| --- | --- | --- |
| MoveC | 不支持 | parser 识别为 `unsupported-syntax` 后跳过（`parseMainBody`/`skipUnsupportedControl`） |
| MoveAbsJ | 不支持 | 同上 |
| 控制流 IF/WHILE/FOR | 不支持 | 首期仅报 unsupported，执行层不解释（`parseMainBody`/`skipUnsupportedControl`；`map.md` "Out of scope"） |
| 非 main 过程 / 函数 | 不支持 | `parseProcedure`/`skipProcedureBody` |
| 变量 `VAR` | 不支持 | `parseRapidProgram` 模块内容分支（非 CONST/PERS/TASK） |
| 非 robtarget 类型 | 不支持 | `KNOWN_UNSUPPORTED_TYPES`/`parseRobTargetDeclaration` |
| Zone 过渡（zone 非 fine） | 不支持 | 运动校验、`plan-shared.ts` |
...
| WaitTime / I/O（SetDO/PulseDO/AO）/ 通信 | 不支持 | 全仓库 grep 无实现；parser 未知语句一律 `unsupported-syntax`（`parseMainBody`） |
| 程序级暂停/继续 | 不支持（无此状态） | `program-executor.ts:8`；`program-control.ts:31` |
| 块注释 / 多行注释 | 不支持 | 词法只处理单行 `!`（`lex`） |
...
- 诊断代码全集 = `RapidDiagnosticCode` 的 9 种，severity 恒 error。

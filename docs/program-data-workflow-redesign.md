...
> ⚠️ **已被取代**：本方案描述的“当前位置 `*` 自动建点并插入运动指令”等“教学简化”条目已被后续《示教工作流对齐真实 FlexPendant》方案取代（建立在真机 IRC5 3HAC050941 操作上：新指令目标为 `*` 未示教占位、参数编辑器补全、Program Data 新建为默认值）。本文档仅作历史记录；新实施以下述现状为准：
>
> - 添加指令：只列 MoveJ/MoveL，插入 `MoveJ|MoveL *,v1000,z50,tool0;`（目标为未示教占位，不自动录当前位置）。
> - 参数编辑：参数面板可改目标点/速度/转弯区；修改位置从参数面板或 Program Data 走同一 `modify-position`。
> - Program Data 新建点位：以类型默认值 `DEFAULT_ROBTARGET` 创建，位置稍后用 Modify Position 录入。
> - 撤销/重做覆盖受控编辑（3 步）。
>
> <details>
> <summary>（历史）原方案摘要</summary>
> 本方案（destination 第 2、4 条与阶段 B 的“当前位置 `*` 一次性建点”设计）已不适用，仅作回溯。
> </details>
